...
{
	return cb * (SMMU500_SMMU_CB1_SCTLR - SMMU500_SMMU_CB0_SCTLR);
}
...
bool smmu_map(bool s1, bool s2, bool write, uintptr_t va, uintptr_t *pa)
{
	...
	reg64_write(addr, v);
	mb();
	reg32_waitformask(m->base + SMMU500_SMMU_GATSR, 1, 0);
	par = reg64_read(m->base + SMMU500_SMMU_GPAR);
	err = par & 1;
	if (!err) {
		*pa = par & ~((1ULL << 12) - 1);
		*pa &= ((1ULL << 48) - 1);
...
void smmu_setup_tcr(struct smmu *m)
{
	...
	for (i = 0; i < 2; i++) {
		...
		writel(m->base + cb_offset(m->cb[i]) + SMMU500_SMMU_CB0_TCR_LPAE,
				tcr);
		writel(m->base + cb_offset(m->cb[i]) + SMMU500_SMMU_CB0_TCR2,
				tcr >> 32);
	}
	mb();
}
...
void smmu_setup_mair(struct smmu *m)
{
	...
	writel(m->base + cb_offset(m->cb[0]) + SMMU500_SMMU_CB0_PRRR_MAIR0,
		m->ctx[0]->mair);
	mb();
}
			...
			unsigned int el, bool enable)
{
	...
	writel(m->base + SMMU500_SMMU_CBA2R0, SMMU500_SMMU_CBA2R0_VA64_MASK);

	r = readl(m->base + SMMU500_SMMU_SCR0);
	r &= ~SMMU500_SMMU_SCR0_CLIENTPD_MASK;
	writel(m->base + SMMU500_SMMU_SCR0, r);

	r = readl(m->base + SMMU500_SMMU_S2CR0);
	r &= ~SMMU500_SMMU_S2CR0_TYPE_MASK;
	writel(m->base + SMMU500_SMMU_S2CR0, r);

	writel(m->base + SMMU500_SMMU_SCR1, 0x00000101);
	...
	if (enable && m->ctx[0]) {
		...
		reg32_write64le(cb_base + SMMU500_SMMU_CB0_TTBR1_LOW,
				(uintptr_t) m->ctx[1]->pt.root);

		writel(m->base + SMMU500_SMMU_CBAR0 + s1_cb * 4,
			s2_cb << 8 |
			3 << SMMU500_SMMU_CBAR0_TYPE_SHIFT);

		mb();

		r = readl(cb_base + SMMU500_SMMU_CB0_SCTLR);
		D(printf("SMMU: SCTLR %x - >%x\n", r,
			r | SMMU500_SMMU_CB0_SCTLR_M_MASK));
		r |= SMMU500_SMMU_CB0_SCTLR_M_MASK;
		writel(cb_base + SMMU500_SMMU_CB0_SCTLR, r);
		mb();
	}
	...
	writel(m->base + SMMU500_SMMU_STLBIALL, 1);
	writel(m->base + SMMU500_SMMU_TLBIALLNSNH, 1);
	mb();
	writel(m->base + SMMU500_SMMU_STLBGSYNC, 1);
	mb();
	reg32_waitformask(m->base + SMMU500_SMMU_STLBGSTATUS, 1, 0);
}
...
static void smmu_probe_init(struct smmu *m)
{
	...
	r = readl(m->base + SMMU500_SMMU_SIDR0);
	...
	r = readl(m->base + SMMU500_SMMU_SIDR1);
	...
	r = readl(m->base + SMMU500_SMMU_SIDR2);
	printf("SMMU: r=%x 64k=%d 16k=%d 4k=%d ubs=%d oas=%d ias=%d\n", r,
		...
		reg32_field(r, SMMU500, SMMU_SIDR2, PTFSV8_4KB),
	...
	printf("SMMU: idr[]=%x, %x, %x\n",
			readl(m->base + SMMU500_SMMU_SIDR0),
			readl(m->base + SMMU500_SMMU_SIDR1),
			readl(m->base + SMMU500_SMMU_SIDR2));
}
